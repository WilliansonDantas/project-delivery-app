...
import { order, orderByOrderID, orderEmTransito } from './mocks/order.mock';
...
describe('Testando a tela de Orders de um costumer',() => {
  ...
  it('Deve possuir uma ordem ao entrar na tela de ordem com Status "Em Trânsito" o usuário Zé Birita', async() => {
    getData.mockResolvedValueOnce(orderEmTransito)
    getData.mockResolvedValue(orderByOrderID)
    ...
    userEvent.click(orderIdButton)

    await waitFor(() => expect(history.location.pathname).toEqual('/customer/orders/1') )

    const deliveryIsDoneBtn = screen.getByTestId('customer_order_details__button-delivery-check')
    expect(deliveryIsDoneBtn).toBeInTheDocument()
    
    userEvent.click(deliveryIsDoneBtn)
  })
